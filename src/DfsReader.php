...
class DfsReader {
	...
	*/
	private function _getBytesFromImage(int $iStart,int $iLen):string
	{
	...
	*/
	 ...
	 * @return int
	*/
	private function _decode18bitAddr(int $iLowByte,int $iMidByte,int $iHighByte,int $iFirstBit,int $iSecondBit):int
	{
		switch($iFirstBit){
			case 1:
	 ...
	 * @return int
	*/
	private function _decode10bitAddr(int $iLowByte,int $iHighByte,int $iFirstBit,int $iSecondBit):int
	{
		switch($iFirstBit){
			case 1:
				$iHighByte = $iHighByte & 3;
	 ...
	 * @param int $iLength
	*/
	private function _getRawData(int $iStartSector,int $iLength):string
	{
	...
	*/
	 ...
	 * @return string
	*/
	public function getFile(string $sFilePath):string
	{
	...
	/**
	 * Gets the stats for the files (as displayed by *EXAMIN)
	 *
	 * @param string $sFilePath
	 * @return array
	*/
	public function getStat(string $sFilePath):array
	{
		$aCat = $this->getCatalogue();
		$aParts = explode('.',$sFilePath);
		if(count($aParts)==2){
	...
	*/
	public function isFile(string $sFilePath):bool
	{
		$aCat = $this->getCatalogue();
		$aParts = explode('.',$sFilePath);
		if(count($aParts)==2){
	...
	*/
	public function isDir(string $sFilePath):bool
	{
		$aCat = $this->getCatalogue();
		$aParts = explode('.',$sFilePath);
		if(count($aParts)==2){
